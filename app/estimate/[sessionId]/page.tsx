  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        {!confirmed && <section className="mt-5 rounded-2xl border border-cyan-200 bg-cyan-50 p-5"><h2 className="font-black text-slate-950">Save your estimate</h2>{submitted ? <p className="mt-2 text-slate-700">Your estimate is saved. A mover will confirm your price within 2 hours.</p> : <><p className="mt-2 text-sm text-slate-600">Your email helps a mover coordinate your confirmation.</p><div className="mt-4 flex flex-col gap-3 sm:flex-row"><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className="min-h-11 flex-1 rounded-xl border border-slate-300 bg-white px-3" /><button type="button" onClick={() => void submitEmail()} disabled={sending} className="min-h-11 rounded-xl bg-cyan-700 px-4 font-bold text-white disabled:opacity-60">{sending ? 'Saving...' : 'Save my estimate'}</button></div></>}</section>}
