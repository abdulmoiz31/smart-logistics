  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <section className="mt-8"><h2 className="text-lg font-black text-slate-950">Looks good</h2><div className="mt-3 space-y-6">{session.rooms.map((room) => <div key={room.id}><div className="mb-2 flex items-center justify-between"><h3 className="font-bold capitalize text-slate-800">{room.roomType.replace('_', ' ')}</h3><button type="button" onClick={() => setPickerRoom(room)} className="min-h-11 text-sm font-bold text-cyan-700">Add item</button></div><div className="space-y-3">{room.items.map((item) => <ItemRow key={item.id} item={item} busy={busyItemId === item.id} onChange={(patch) => void updateItem(item, patch)} onRemove={() => void deleteItem(item)} />)}</div></div>)}</div></section>
