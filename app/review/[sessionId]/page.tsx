...
export default function ReviewPage() {
  ...
  if (!session) return <main className="grid min-h-screen place-items-center bg-slate-50 p-6"><div className="max-w-sm text-center"><p className="font-semibold text-rose-700">{error || 'This scan could not be found.'}</p><Link href="/" className="mt-4 inline-block font-bold text-cyan-700">Start a new scan</Link></div></main>;
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-2 text-slate-600">{uncertain.length ? "We've surfaced anything uncertain first. Tap through changes -- the estimate updates after review." : 'Review the items below, then get your estimate.'}</p>
        {error && <p role="alert" className="mt-4 rounded-xl bg-rose-50 p-3 text-sm font-semibold text-rose-800">{error}</p>}
        {uncertain.length > 0 && <section className="mt-7"><h2 className="text-lg font-black text-slate-950">We weren&apos;t sure about these</h2><p className="mt-1 text-sm text-slate-600">A quick check helps keep your estimate honest.</p>
          {quotaBlock && (
            <div className="mt-3 rounded-2xl border border-amber-300 bg-amber-50 p-5">
              {quotaBlock.authenticated ? (
                <div>
                  <p className="font-black text-amber-950">Daily limit reached (15 checks)</p>
                  <p className="mt-1 text-sm text-amber-900">Your allowance resets tomorrow.</p>
                </div>
              ) : (
                <div>
                  <p className="font-black text-amber-950">You&apos;ve used your 3 free AI checks today.</p>
                  <p className="mt-1 text-sm text-amber-900">Create a free account to check 15 items a day. It takes a few seconds.</p>
                  <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                    <Link href={`/signup?next=/review/${sessionId}`} className="grid min-h-11 place-items-center rounded-xl bg-amber-600 px-4 font-bold text-white">Create free account</Link>
                    <Link href={`/login?next=/review/${sessionId}`} className="grid min-h-11 place-items-center font-semibold text-amber-900">Already have an account? Sign in</Link>
                  </div>
                </div>
              )}
            </div>
          )}
          <div className="mt-3 space-y-3">{uncertain.map(({ item }) => <div key={item.id} className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-slate-900">{formatLabel(item.name)}</p><p className="mt-1 text-sm text-slate-600">{Math.round(item.confidence * 100)}% confidence</p>{item.uncertaintyReason && <p className="mt-0.5 text-xs text-slate-500">{item.uncertaintyReason}</p>}</div><button type="button" disabled={busyItemId === item.id || Boolean(quotaBlock)} onClick={() => void refine(item)} className="min-h-11 rounded-xl bg-amber-500 px-3 text-sm font-bold text-slate-950 disabled:opacity-50">{busyItemId === item.id ? 'Checking...' : 'Check this'}</button></div></div>)}</div></section>}
        <section className="mt-8 space-y-5">{session.rooms.map((room) => {
          ...
          const addButton = <button type="button" onClick={() => setPickerRoom(room)} className="min-h-11 text-sm font-bold text-cyan-700">Add item</button>;

          if (!confirmedItems.length) {
            return <div key={room.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between gap-3"><div><h2 className="font-bold text-slate-900">{roomLabel}</h2><p className="mt-1 text-sm text-slate-600">All items for this room are shown above for review.</p></div>{addButton}</div></div>;
          }

          return <details key={room.id} open={session.rooms.length === 1} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><summary className="cursor-pointer font-black text-slate-950">{roomLabel} · {confirmedItems.length} confirmed item{confirmedItems.length === 1 ? '' : 's'} · {Math.round(roomCubicFeet * 10) / 10} cu ft</summary><div className="mt-4"><div className="mb-2 flex justify-end">{addButton}</div><div className="space-y-3">{confirmedItems.map((item) => <ItemRow key={item.id} item={item} busy={busyItemId === item.id} onChange={(patch) => void updateItem(item, patch)} onRemove={() => void deleteItem(item)} />)}</div></div></details>;
        })}</section>
